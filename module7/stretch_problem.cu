...
__host__ void generate_rand_data(unsigned int *data, unsigned int num_elements) {
    for (unsigned int i = 0; i < num_elements; i++) {
        // CHANGED HERE
        data[i] = rand(); // simple call to rand
    }
}
...
void print_all_CUDA_devices_and_properties() {
    // CHANGED HERE
    /* device_id was used instead of device count, and the method for printing
     * device properties was call on incorrect device ordinals, resulting in error */
...
__host__ float execute_shared_memory_operations() {
    ...
    // CHANGED HERE
    /* dont need to free host mem because it is never malloced into existence,
     * so it can't be free'd with any sort of free call */
    //free device and host memory allocations
    cudaFree((void *) d_data);        //free device data
    cudaDeviceReset();
...
int main(void) {
    ...
    // CHANGED HERE -- call the functions
