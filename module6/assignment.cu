...
#define DEFAULT_KERNEL_LOOP_ITERATIONS 1024
...
// the standard quadKernel using registers memory
__global__ void quadKernelRegisters(int *inputA, int *inputB, int *output, int loop_iterations) {
    int threadID = blockIdx.x * blockDim.x + threadIdx.x;

    int regA = inputA[threadID];
    int regB = inputB[threadID];
    ...
    output[4 * threadID] = regOut1;
    output[4 * threadID + 1] = regOut2;
    output[4 * threadID + 2] = regOut3;
    output[4 * threadID + 3] = regOut4;
}

__host__ float
doQuadKernelGlobal(int *inputA, int *inputB, int *output, int totalThreads, int blockSize, int loop_iterations) {
...
__host__ float
doQuadKernelShared(int *inputA, int *inputB, int *output, int totalThreads, int blockSize, int loop_iterations) {
    int numBlocks = totalThreads / blockSize;
...
__host__ float
doQuadKernelConstant(int *inputA, int *inputB, int *output, int totalThreads, int blockSize, int loop_iterations) {
    int numBlocks = totalThreads / blockSize;
...
__host__ float
doQuadKernelRegisters(int *inputA, int *inputB, int *output, int totalThreads, int blockSize, int loop_iterations) {
    int numBlocks = totalThreads / blockSize;
    ...
    // execute
    quadKernelRegisters<<<numBlocks, blockSize>>>(device_inputA, device_inputB, device_output, loop_iterations);
    cudaDeviceSynchronize();
...
__host__ void executeComparison(int totalThreads, int blockSize, int loop_iterations) {
    ...
    cudaMallocHost(&registerOutput, 4 * totalThreads * sizeof(*registerOutput));
    ...
    float registerTime = doQuadKernelRegisters(inputA, inputB, registerOutput, totalThreads, blockSize, loop_iterations);

    // verify the outputs
    assert(validateResults(globalOutput, sharedOutput, 4 * totalThreads));
    assert(validateResults(globalOutput, registerOutput, 4 * totalThreads));
    ...
    printf("Four operation kernel time... registers: %f (ms)\n", registerTime);
